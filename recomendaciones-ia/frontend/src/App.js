...
import './App.css';
// ❌ Ya no usamos el archivo JSON local, se comenta para evitar conflictos
...
function App() {
  ...
  const handleBuscarTexto = () => {
    ...
    const filtradas = todas.filter((peli) =>
  ...
  const handleBuscarDescripcion = async () => {
    ...
    try {
      ...
      //const response = await fetch('/api/recomendaciones', {
        ...
        body: JSON.stringify({
          prompt: `Tengo una base de datos con estas películas: ${todas.map(p => p.titulo).join(', ')}. 
...
Devuélveme únicamente los títulos separados por comas.`
      ...
      const textoIA = data.recomendacion.replace(/<[^>]*>/g, '').toLowerCase();

      setRecomendacionIA(data.recomendacion);

      const coincidencias = todas.filter((peli) =>
        ...
        <div className="galeria">
          ...
          <div className="grid">
            {peliculasRecomendadas.map((peli) => (
              <div className="tarjeta" key={peli._id}>
